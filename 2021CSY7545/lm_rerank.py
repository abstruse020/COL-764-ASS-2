...
# from random import randint
#%% Initializing Paths
relevence_path = 'GivenData/t40-qrels.txt'
...
class rm1:
    ...
    def expansion_term(self, query, q_id, query_str, cord_ids):
        ...
        #     random_index = randint(0, v-1)
        #     prob_list[random_index] = self.P_w_q(
        #         self.vocab.to_word(random_index),
        #         query_str,
        #         self.models[q_id]
        #         )
...
class rm2:
    ...
    def expansion_term(self, query, q_id, query_str, cord_ids):
        ...
        for i in range(v):
            prob_list[i] = self.P_word_givn_R(
                self.vocab.to_word(i),
                query_str,
                self.models[q_id]
                )
        #     prob_list[i] /= p_qs 
        #     if i%1000 == 0:
        #         print("{0:1.2f}%".format(i*100/v), end=' ')
        # # Not using random way to pic data
        # for i in range(10000):
        #     random_index = randint(0, v-1)
        #     self.P_word_givn_R(self.vocab.to_word(random_index), query_str, self.models[q_id])
...
def main():
    ...
    if rm_model == 'rm1':
        mu = 0.1
        print('mu', mu)
        r1 = rm1(mu, vocab, docs, doc_models)
        r1.process_queries(queries, docs_by_topic, output_path)
        
    if rm_model == 'rm2':
        mu = 0.5
        print('mu:', mu)
        r2 = rm2(mu, vocab, docs, doc_models)
